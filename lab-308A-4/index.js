...
const API_KEY = "live_92QsgmenerVyDjykZgwa3ZwyxNY2J9XZISpKsLEUvpGndNeM1osQ1D7i4Bbl8C9N";
...
export async function favourite(imgId) {
  try {
    ...
    const isFavorited = response.data.find(favourite => favourite.image_id === imgId);
    ...
    if (isFavorited) {
      ...
      const favouriteData = await axios.delete(`https://api.thecatapi.com/v1/favourites/${isFavorited.id}`, {
        headers: {
          'x-api-key': API_KEY
        }
      });
      ...
      const favData = await axios.post('https://api.thecatapi.com/v1/favourites', {
        image_id: imgId
      }, {
        headers: {
          // method: 'POST',
...
async function getFavourites(event) {
  try {
    // const breedId = event.target.value;
    // console.log(breedId)
    const favouritesData = await axios.get(`https://api.thecatapi.com/v1/favourites`, {
      headers: {
        'x-api-key': API_KEY,
    ...
    favouritesData.data.forEach(async (favourite) => {
      if (favourite.image_id.length > 4) {
        const item = Carousel.createCarouselItem(favourite.image.url, "alt", favourite.image_id)
        Carousel.appendCarousel(item);
      }
    })
